<?php
//
//  Module: getNewAuthorsWithQuotes.php - G.J. Watson
//    Desc: Get an author and their quotes to the requestor as a Json response based on a 'newer thank time' parameter
// Version: 1.01
//

function getNewAuthorsWithQuotes($db, $fromDateTime) {
    $arr = [];
    $authors = $db->select(getNewAuthorsSQL($fromDateTime));
    while ($row = $authors->fetch_array(MYSQLI_ASSOC)) {
        ...
        $quotes = $db->select(getNewAuthorQuotesSQL($author->getAuthorID(), $fromDateTime));
